...
public class SearchBox extends RelativeLayout {
	...
	public void setOnSuggestionClickListener(OnSuggestionClick listener) {
		this.onSuggestionClick = listener;
	}
	...
	private void addResult(SearchResult result) {
		if (resultList != null && resultList.size() < 6 && !resultList.contains(result)) {
	...
	private void search(SearchResult result) {
		...
		if (!TextUtils.isEmpty(getSearchText())) {
			...
			if (onSuggestionClick != null)
				onSuggestionClick.onSuggestionClick(result);
			addResult(result);
		} else {
			setLogoTextInt(logoText);
		}
	...
	private void search(String text) {
		if (searchables.size() > 0) {
			SearchResult option = searchables.get(0);
			search(option);
		}
	}
